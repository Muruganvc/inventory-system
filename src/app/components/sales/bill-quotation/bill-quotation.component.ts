...
import { BillQuotationDialogComponent } from './bill-quotation-dialog/bill-quotation-dialog.component';

@Component({
  selector: 'app-bill-quotation',
  standalone: true,
...
export class BillQuotationComponent {
  ...
  onPrint = (data: any): void => {
    ...
    this.productSales.forEach(item => {
      grandTotal += item.meter > 0 ? item.meter * item.price : item.quantity * item.price;
    });

    // Company & Customer Info (replace with your actual data)
    const companyInfo = `
    <div style="text-align:center; margin-bottom:20px;">
      <h1>VENNILA ELECTRICALS</h1>
      <p>Theerthamalai, Harur</p>
      <p>Phone: +91-99942-77980 | Email: vcmuruganmca@email.com</p>
    </div>
  `;
  ...
  `;
    ...
    // Generate table dynamically from the list
    let content = `<h2>Product Details</h2>
    <table style="width:100%; border-collapse: collapse;">
      <thead>
        <tr>
          <th style="border: 1px solid #000; padding: 4px;">Name of Product(s)</th>
          <th style="border: 1px solid #000; padding: 4px;">Qty / Meter</th>
    ...
    this.productSales.forEach(item => {
      const total = item.meter > 0 ? item.meter * item.price : item.quantity * item.price;
      content += `<tr>
      <td style="border: 1px solid #000; padding: 4px;">${item.productName}</td>
      <td style="border: 1px solid #000; padding: 4px;">${item.meter > 0 ? item.meter : item.quantity}</td>
      <td style="border: 1px solid #000; padding: 4px;">₹${item.price.toFixed(2)}</td>
      <td style="border: 1px solid #000; padding: 4px;">₹${total.toFixed(2)}</td>
    </tr>`;
    });
    ...
    <tfoot>
      <tr>
        <td colspan="3" style="border:1px solid #000; padding:4px; text-align:right;"><strong>Grand Total</strong></td>
        <td style="border:1px solid #000; padding:4px;"><strong>₹${grandTotal.toFixed(2)}</strong></td>
      </tr>
    </tfoot>
  </table>
   ...
   <p style="font-size: 12px; text-align: center; font-style: italic;">Product(s) rate may vary at the time of purchase.</p>
  `;
