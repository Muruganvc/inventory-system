...
export class SalesComponent implements OnInit {
  ...
  private initFields(): void {
    ...
    this.fields = [
      ...
      { type: 'input', name: 'salesPrice', label: 'Sales Price ₹', colSpan: 2, isNumOnly: true, maxLength: 8 },
  ...
  private patchForm(product: ProductsResponse): void {
    this.formGroup.patchValue({
      ...
      totalAmount: this.calculateGSTFromBasePrice(product.salesPrice, product.taxPercent).totalPrice
    });
  }
  ...
  private addProduct(form: any): void {
    ...
    const totalAmount = this.calculateGSTFromBasePrice(form.salesPrice, selectedProduct.taxPercent).totalPrice;
  ...
  handleFieldChange(event: any): void {
    // Future: handle field-level changes
  }
}
